...
public class CommonGoal_SixSeparateGroupsOfTwoObj extends CommonGoal {
	...
	/**
	 * To complete the common Goal the player must create 6 groups with his tiles
	 * from two tiles of the same type, the groups of 2 tiles must not touch each
	 * other therefore for example a pair of two cats cannot have another group of
	 * books,game ecc... adjacent to them.This method checks every time all the
	 * possible combinations(vertical and horizontal combinations) of positions in
	 * which groups of two tiles can be created around the tiles considered,
	 * therefore if there are no tiles forming an adjacent group of two it means
	 * that the tiles considered are isolated, therefore they are saved in an array,
	 * if the array at the end contains 12 or more elements it means that the goal
	 * has been completed.
	 * 
	 * 
					...
					if (RefobjUnder.name() == RefobjUpper.name()) {
						...
						if (!(CoordsBlackList.isEmpty())) {
							for (MatrixCoords black : CoordsBlackList) {
								if (black.equals(new MatrixCoords(CURRENT_ROW, CURRENT_COL)) == true
										|| black.equals(new MatrixCoords(UPPER_ROW, CURRENT_COL)) == true) {
						...
						if (isPresentInBlackList == 0) {
							if (!(IsIsolated(new MatrixCoords(adjacent1_ROW, adjacent_col),
									new MatrixCoords(adjacent2_ROW, adjacent_col), bookshelf))) {
							...
							if (!(IsIsolated(new MatrixCoords(Horizontal_ROW, Horizontal1_COL),
									new MatrixCoords(Horizontal_ROW, Horizontal2_COL), bookshelf))) {
								CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_COL));
								CoordsBlackList.add(new MatrixCoords(UPPER_ROW, CURRENT_COL));
							...
							if (!(IsIsolated(new MatrixCoords(HorizontalUP_ROW, HorizontalUP1_COL),
									new MatrixCoords(HorizontalUP_ROW, HorizontalUP2_COL), bookshelf))) {
								CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_COL));
								CoordsBlackList.add(new MatrixCoords(UPPER_ROW, CURRENT_COL));
							...
							if (row >= 2) {
								if (!(IsIsolated(new MatrixCoords(HorizontalUpper_ROW, HorizontalUpper1_COL),
										new MatrixCoords(HorizontalUpper_ROW, HorizontalUpper2_COL), bookshelf))) {
									CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_COL));
									CoordsBlackList.add(new MatrixCoords(UPPER_ROW, CURRENT_COL));
							...
							if (row >= 3) {
								if (!(IsIsolated(new MatrixCoords(VerticalUpper1_ROW, VerticalUpper_COL),
										new MatrixCoords(VerticalUpper2_ROW, VerticalUpper_COL), bookshelf))) {
									CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_COL));
									CoordsBlackList.add(new MatrixCoords(UPPER_ROW, CURRENT_COL));
							...
							if (col == 4 && row >= 3) {
								if (!(IsIsolated(new MatrixCoords(HorizontalUpper_ROW, HorizontalUpper1_COL),
										new MatrixCoords(HorizontalUpper_ROW, HorizontalUpper2_COL - 2), bookshelf))) {
									CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_COL));
									CoordsBlackList.add(new MatrixCoords(UPPER_ROW, CURRENT_COL));

									CoordsBlackList.add(new MatrixCoords(HorizontalUpper_ROW, HorizontalUpper1_COL));
									CoordsBlackList
											.add(new MatrixCoords(HorizontalUpper_ROW, HorizontalUpper2_COL - 2));
									isISOLATED = false;
								}
			...
			for (int col = 0; col < bookshelf.getCols() - 1; col++) {
				...
				if (RefobjUnder != null && RefobjUpper != null) {
					if (RefobjUnder.name() == RefobjUpper.name()) {
						...
						if (!(CoordsBlackList.isEmpty())) {
							for (MatrixCoords black : CoordsBlackList) {
								if (black.equals(new MatrixCoords(CURRENT_ROW, CURRENT_L_COL)) == true
										|| black.equals(new MatrixCoords(CURRENT_ROW, CURRENT_R_COL)) == true) {
									isPresentInBlackList = 1;
								}
							}
						}
						...
						if (isPresentInBlackList == 0) {
							...
							if (row >= 3) {
								if (!(IsIsolated(new MatrixCoords(VerticalUpper1_L_ROW, VerticalUpper_L_COL),
										new MatrixCoords(VerticalUpper2_L_ROW, VerticalUpper_L_COL), bookshelf))) {
									CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_L_COL));
									CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_R_COL));
							...
							if (row >= 3) {
								if (!(IsIsolated(new MatrixCoords(VerticalUpper1_R_ROW, VerticalUpper_R_COL),
										new MatrixCoords(VerticalUpper2_R_ROW, VerticalUpper_R_COL), bookshelf))) {
									CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_L_COL));
									CoordsBlackList.add(new MatrixCoords(CURRENT_ROW, CURRENT_R_COL));
	...
	/**
	 * This method checks if an object is isolated from another object of the same
	 * type.
	 * 
	 * @param cords1    first object coordinates
	 * @param cords2    first object coordinates
	 * @param bookshelf the player bookshelf under validation
	 * @return true if the two objects are different, else false
	 */
